...
class SemanticSearch:
    ...
    def tokenize_with_specb(self, texts, is_query):
        ...
        for seq, att in zip(batch_tokens["input_ids"], batch_tokens["attention_mask"]):
            ...
            att.append(1)
        # Add padding
        batch_tokens = self.tokenizer.pad(batch_tokens, padding=True, return_tensors="pt").to(self.model.device)
        return batch_tokens
...
class CocoSearch:
    ...
    def encode(self, texts, is_query=False):
        with torch.no_grad():
            ...
            embeddings = self.model(**inputs, output_hidden_states=True, return_dict=True).hidden_states[-1][:, :1].squeeze(1)
